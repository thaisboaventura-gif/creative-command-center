...
export async function GET(req: Request) {
  try {
    ...
    // Support ?key=BDSL-XXXXX for single-issue lookup
    ...
    // Active parent tasks with D2C in the title
    const jql1 = `project = ${project} AND summary ~ "D2C" AND issuetype not in subTaskIssueTypes() AND statusCategory != Done AND status != Backlog ORDER BY updated DESC`;

    // Done parent tasks with D2C this year
    const jql2 = `project = ${project} AND summary ~ "D2C" AND issuetype not in subTaskIssueTypes() AND statusCategory = Done AND created >= "${currentYear}-01-01" ORDER BY updated DESC`;
